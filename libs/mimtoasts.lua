...
local newFrame = imgui.OnFrame(
    ...
    function(self)
        ...
        imgui.SetWindowPosVec2(imgui.ImVec2(resX - 10 - winSize.x, resY - 200))
        ...
        for k, data in ipairs(list) do
            ...
            local iconSize = imgui.CalcTextSize(EXPORTS.ICON[data.type] or fa.CIRCLE_INFO)
            local off = 10
            local size = imgui.ImVec2(off + iconSize.x + off + textSize.x + off, off + textSize.y + off)
            ...
            imgui.BeginChild('toastNotf:'..tostring(k)..tostring(data.text), size, true, imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoScrollWithMouse)
                ...
                imgui.SetCursorPos(imgui.ImVec2(off, size.y / 2 - iconSize.y / 3))
                imgui.Text(EXPORTS.ICON[data.type] or fa.CIRCLE_INFO)
                imgui.PopStyleColor()

                imgui.PushStyleColor(imgui.Col.Text,    tableToImVec(data.colors.text or COLORS[data.type].text))
                imgui.SetCursorPos(imgui.ImVec2(off + iconSize.x + off, size.y / 2 - textSize.y / 2))
